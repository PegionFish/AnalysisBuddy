//! 插件管理命令逻辑体（ipc-ui.md §1.1 / §2.2 / §4.6；M1 自
//! core/ab-app/src/commands/plugin.rs 机械迁入）：`list_plugins`（8 命令
//! 之一）、辅助命令 `get_plugin_log`（stderr 环形缓冲尾部补发）与
//! `reload_plugin`（停机后重建实例，返回新 `PluginInfo`）的 `*_logic`
//! 纯函数与测试。Tauri `#[tauri::command]` 薄包装留在 core/ab-app。
