//! 会话命令逻辑体（ipc-ui.md §1.7/§1.8；M1 自 core/ab-app/src/commands/
//! session.rs 机械迁入）：`save_session` / `load_session` 的 `*_logic` 纯
//! 函数与测试（显式 path 注入）。Tauri `#[tauri::command]` 薄包装、系统
//! 另存为对话框（`pick_save_path`）与 `path` 省略回退留在 core/ab-app
//! （原 `save_session`：`path` 省略时调系统另存为对话框，取消 → reject
//! `cancelled`；`load_session` 校验 missing/hash_mismatch，通过者按记录
//! plugin_id 重走导入管线，pipeline.md §5.3）。
