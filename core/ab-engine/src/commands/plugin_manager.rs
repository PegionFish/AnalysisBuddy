//! 模块管理命令逻辑体（spec §4.1/§4.2/§4.4/§5.1/§5.2；M1 自
//! core/ab-app/src/commands/plugin_manager.rs 机械迁入）：
//! `install_plugin_zip`、`uninstall_plugin`、`set_plugin_enabled`
//! （T6 追加 check/update）的 `*_logic` 纯函数、模块状态文件处理与测试。
//! Tauri `#[tauri::command]` 薄包装与 `default_plugins_dir()` 路径公式
//! 留在 core/ab-app（plugins_dir 由调用方注入）。
//!
