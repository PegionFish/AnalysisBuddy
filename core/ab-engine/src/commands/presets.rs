//! 用户预设存储逻辑体（Wave 2 C5；M1 自 core/ab-app/src/commands/
//! presets.rs 机械迁入）：`list_user_presets` / `save_user_preset` /
//! `delete_user_preset` 的 `*_logic` 纯函数与测试，预设目录由调用方注入
//! （`&Path`）。Tauri `#[tauri::command]` 薄包装与 `presets_dir()` 路径
//! 公式留在 core/ab-app，文件 `%APPDATA%\AnalysisBuddy\presets\<id>.abpreset.json`
...
mod tests {
    ...
    async fn concurrent_delete_and_save_same_id_serialize() {
        ...
        if let Some(p) = files.first() {
            ...
            serde_json::from_slice::<serde_json::Value>(&raw).expect("残留文件必须为完整合法 JSON");
        }
        fs::remove_dir_all(&dir).expect("cleanup");
    }
}
