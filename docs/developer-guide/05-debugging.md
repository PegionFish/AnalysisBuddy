...
| 预设不显示/部分丢失；`plugin check` 输出 MAN-14 警告 | `MAN-14` | `presets` 存在重复预设/分组 id（宿主仅保留首个）；或 `name`/`description`/`names`/`keywords` 有 trim 后为空的字符串（宿主丢弃该预设/分组/条目） | 预设与分组 id 改为唯一；双语名、候选名与 keyword 去掉纯空白项，见 [04-manifest-reference.md](04-manifest-reference.md)「presets（场景预设）」 |
