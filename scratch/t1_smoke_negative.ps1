# T1 negative-path check: smoke must FAIL on lowercase fingerprints (P2 regression)
...
$tmp = Join-Path $env:TEMP 't1-negative-12656'
$fixture = Join-Path (Get-Location) 'plugins\demo-tool\tests\fixtures\small_txt.log'
try {
    Invoke-DemoToolSmoke $tmp $fixture
    Write-Output 'NEGATIVE_PATH_NOT_CAUGHT'
    exit 1
} catch {
    Write-Output ('NEGATIVE_PATH_CAUGHT: ' + $_.Exception.Message)
    exit 0
}
