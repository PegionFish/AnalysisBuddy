...
规划正文（§1–§9）定稿后发生的架构级变更在此追加记录；修改须经主代理评审。

| 日期 | 决策 | 理由 / 说明 |
|------|------|-------------|
| 2026-09-06 | 自 `core/ab-app` 提取纯 Rust headless 引擎 `core/ab-engine`（M1：命令 `*_logic` 纯函数 + DTO、导入编排、事件转换、宿主桥接、更新网络、冒烟），并新增 `core/ab-server`（M2：axum HTTP+SSE 服务，契约见 `docs/spec/http-api-v1.md`）。产品交付形态扩为三种：**Windows EXE 桌面版 / Linux 服务器版 / 嵌入式引擎库**，三者共用同一引擎核心与同一套插件。HTTP API 面向厂商中立：不绑定 Tauri/桌面私有契约，响应 DTO 与桌面 ipc-ui.md §1.0 形状逐字段平价，服务器侧新增语义（异步 job、SSE、`file_ids` 缺省）均在 http-api-v1.md 显式文档化。 | 服务器/嵌入式交付要求引擎与 UI 解耦；单一引擎核心避免三形态分叉；插件生态（manifest/协议 v1）零变更即跨形态复用。 |
