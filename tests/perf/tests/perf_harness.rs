...
use ab_perf::report::{regression_check, Metrics, PerfReport};
use ab_perf::rss;
...
fn thresholds_full_and_smoke() {
    ...
    // 5 次采样中位数判据。
    let r = judge_median(&[0.8, 0.9, 1.2, 0.7, 0.6], Some(280.0), &[25.0, 30.0, 22.0, 28.0, 29.0], None, &smoke);
...
fn rss_sampler_dual_path_deviation_within_5pct() {
    ...
    let script = ws.join("tests").join(".generated").join("rss_dual_probe.ndjson");
    ...
    let ps_out = Command::new("powershell")
        .args([
            ...
            "Bypass",
            "-File",
            &ws.join("tests").join("perf").join("rss_probe.ps1").to_string_lossy(),
            "-ProcessId",
            &pid.to_string(),
            "-Seconds",
            "6",
...
fn report_schema_frozen_and_regression_drill() {
    ...
    // 演练记录入仓（供回归演练证据）。
    let drill = workspace_root().join("tests").join("perf").join("reports").join("regression-drill.md");
...
fn debug_build_data_never_goes_to_report() {
    ...
    let smoke = Thresholds::smoke_10mb();
    assert!(judge(Some(0.9), Some(200.0), Some(30.0), Some(35.0), &smoke)[..3] == [true, true, true]);
}
